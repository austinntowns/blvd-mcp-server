...
async function main() {
  ...
  // Introspect ListOfStaffShifts type
  const query = gql`
    query IntrospectListOfStaffShifts {
      __type(name: "ListOfStaffShifts") {
        name
        fields {
          name
          type {
  ...
  console.log("ListOfStaffShifts type fields:");
  console.log(JSON.stringify(result, null, 2));

  // Also check ShiftInstance or similar types
  const query2 = gql`
    query FindShiftTypes {
      __schema {
        types {
          name
          kind
  ...
  const shiftTypes = (result2 as any).__schema.types.filter(
    (t: any) => t.name.toLowerCase().includes("shift") || t.name.toLowerCase().includes("schedule")
  );
  console.log("\nShift-related types:");
  console.log(JSON.stringify(shiftTypes, null, 2));
}
